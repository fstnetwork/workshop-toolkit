      ...
      .then(pp => {
        if (pp) {
          ...
          window.$("#address").on("click", () => {
            window.open(
              globalEndpointExplorer + "/address/" + pp.checksumAddressString,
              "_blank"
            );
          });
    ...
    window
      ...
      .then(resp => {
        if (!resp.data.data.submitTransaction) {
          ...
          window
            .$("#txhash")
            .val("The transaction is expired or duplicated on DataRail");
...
function reloadHashTable() {
  ...
  const tmpTbody = obj.hashTable.map((rowObj, i) => {
    ...
    if (globalEndpointExplorer !== "") {
      de_identified_entry_link = `<a style="text-decoration-line: underline;" href="${globalEndpointExplorer}/address/0x${rowObj.de_identified_entry}" target="_blank">${rowObj.de_identified_entry}</a>`;
    }
...
window.$(function() {
  ...
  const endpointChangeStream = window
    ...
    .map(e => {
      ...
      // workshop.fst.network/{ce}/api
      if (
        window
          .$(e.target)
          .val()
          .includes("workshop.fst.network") &&
        !window
          .$(e.target)
          .val()
          .includes("api.")
        ...
        const originAndWorkshopName = tmpStr.match(
          /workshop\.fst\.network\/.*?\//
        )[0];
        const endpointUrl = `https://${originAndWorkshopName}api`;

        window.$(e.target).val(endpointUrl);
        ...
        window
          .$("#explorer-preview")
          .html(
            `(The DataRail Explorer for the Data Entry is at: <a id="explorer-preview-link" href="${globalEndpointExplorer}" target="_blank">${globalEndpointExplorer}</a>)`
          );
      }
      // only input {ce}
      else if (
        ...
        let tmpEndpointUrl = window
          .$(e.target)
          .val()
          .trim()
          .split("/")[0];

        const endpointUrl = `https://workshop.fst.network/${tmpEndpointUrl}/api`;
      ...
      // api.?.[fst.dev][fstk.io][fst.network]/?
        ...
        if (tmpEndpointUrl.startsWith("//")) {
          tmpEndpointUrl = `https:${tmpEndpointUrl}`;
        }

        if (
          !(
            tmpEndpointUrl.startsWith("http://") ||
            tmpEndpointUrl.startsWith("https://")
          )
        ) {
          tmpEndpointUrl = `https://${tmpEndpointUrl}`;
        }

        window.$(e.target).val(tmpEndpointUrl);
  ...
  const mutationReqInputStream = window
    .$("#mutation-req")
    .asEventStream("input")
    .map(e => {
      window.$(e.target).removeClass("bad-form");
  ...
  mutationReqInputStream.onValue(r => {
    if (r !== "") {
      window.$("#submit-transaction").prop("disabled", true);
    }
  });
