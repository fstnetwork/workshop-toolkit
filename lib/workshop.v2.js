...
function afterClickSubmitTransactionButton() {
  ...
  const passphrase = window.$("#ethk-pp").val();
  const mutationReq = window.$("#mutation-req").val();
  const mutationResp = window.$("#mutatoin-resp").val();
  const access_token = window.$("#authorization-header").attr("data") || "";
...
window.$(function() {
  ...
  const endpointChangeStream = window
    ...
    .map(e => {
      ...
      window
        .$("#explorer-preview")
        .html(
          `(The Blockchain Explorer for the Data Entry is at: <a id="explorer-preview-link" href="${globalEndpointExplorer}" target="_blank">${globalEndpointExplorer}</a>)`
        );
  ...
  $("#sigin-id").trigger("change");
  $("#sigin-pw").trigger("change");
  $("#ethk-pp").trigger("change");
  $("#mutation-req").trigger("change");
});
