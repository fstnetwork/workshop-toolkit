...
function parseMutationResp(content) {
  ...
  var transaction = jsonpath.query(resp, "$.data.*.transaction")[0];
  var submitToken = jsonpath.query(resp, "$.data.*.submitToken")[0];
...
function getAddressUI() {
  ...
  if ([endpoint, id, pw, passphrase, access_token].every(function (content) {
    ...
    window.axios({
      ...
      window.$("#address").val("Decrypting the key file (wallet file)...");
      ...
      if (pp) {
        ...
        window.$("#address").on("click", function () {
          window.open(globalEndpointExplorer + "/address/" + pp.checksumAddressString, "_blank");
        });
  ...
  if ([endpoint, id, pw, passphrase, mutationReq, mutationResp, access_token, _parsedMutationResp].every(function (content) {
    ...
    window.axios({
      ...
      if (!resp.data.data.submitTransaction) {
        ...
        window.$("#txhash").val("The transaction is expired or duplicated on DataRail");
      ...
      window.$("#check-txhash").on("click", function () {
        window.open(globalEndpointExplorer + "/tx/" + latestTxHash, "_blank");
      }); // set UI (defreeze UI)
...
window.$(function () {
  ...
  var endpointChangeStream = window.$("#endpoint").asEventStream("change").map(function (e) {
    ...
    var tmpEndpoint = ""; // workshop.fst.network/{ce}/api

    if (window.$(e.target).val().includes("workshop.fst.network") && !window.$(e.target).val().includes("api.")) {
      var tmpStr = window.$(e.target).val().trim();
      if (tmpStr[tmpStr.length - 1] !== "/") tmpStr += "/";
      var originAndWorkshopName = tmpStr.match(/workshop\.fst\.network\/.*?\//)[0];
      var endpointUrl = "https://".concat(originAndWorkshopName, "api");
      window.$(e.target).val(endpointUrl); // set UI for explorer OMG

      tmpEndpoint = window.$(e.target).val();
      globalEndpointExplorer = tmpEndpoint.replace("/api", "").replace("https://", "http://").replace("http://", "//");
      reloadHashTable();
      window.$("#explorer-preview").html("(The DataRail Explorer for the Data Entry is at: <a id=\"explorer-preview-link\" href=\"".concat(globalEndpointExplorer, "\" target=\"_blank\">").concat(globalEndpointExplorer, "</a>)"));
    } // only input {ce}
    else if (!window.$(e.target).val().includes(".") && window.$(e.target).val() !== "") {
        var tmpEndpointUrl = window.$(e.target).val().trim().split("/")[0];

        var _endpointUrl = "https://workshop.fst.network/".concat(tmpEndpointUrl, "/api");

        window.$(e.target).val(_endpointUrl); // set UI for explorer OMG

        tmpEndpoint = window.$(e.target).val();
        globalEndpointExplorer = tmpEndpoint.replace("/api", "").replace("https://", "http://").replace("http://", "//");
        reloadHashTable();
        window.$("#explorer-preview").html("(The DataRail Explorer for the Data Entry is at: <a id=\"explorer-preview-link\" href=\"".concat(globalEndpointExplorer, "\" target=\"_blank\">").concat(globalEndpointExplorer, "</a>)"));
      } else if (window.$(e.target).val() === "") {
        return tmpEndpoint;
      } else if (window.$(e.target).val() === "http://") {
        return tmpEndpoint;
      } else if (window.$(e.target).val() === "https://") {
        return tmpEndpoint;
      } // api.?.[fst.dev][fstk.io][fst.network]/?
      else {
          var _tmpEndpointUrl = window.$(e.target).val().replace("fst.network/api", "fst.network").replace("fstk.io/api", "fstk.io").replace("fst.dev/api", "fst.dev").replace("fstk.dev/api", "fstk.dev").replace("fstnetwork.dev/api", "fstnetwork.dev").replace("fst.network/explorer", "fst.network").replace("fstk.io/explorer", "fstk.io").replace("fst.dev/explorer", "fst.dev").replace("fstk.dev/explorer", "fstk.dev").replace("fstnetwork.dev/explorer", "fstnetwork.dev").replace("fst.network/signin", "fst.network").replace("fstk.io/signin", "fstk.io").replace("fst.dev/signin", "fst.dev").replace("fstk.dev/signin", "fstk.dev").replace("fstnetwork.dev/signin", "fstnetwork.dev");

          if (_tmpEndpointUrl.startsWith("//")) {
            _tmpEndpointUrl = "https:".concat(_tmpEndpointUrl);
          }

          if (!(_tmpEndpointUrl.startsWith("http://") || _tmpEndpointUrl.startsWith("https://"))) {
            _tmpEndpointUrl = "https://".concat(_tmpEndpointUrl);
          }

          window.$(e.target).val(_tmpEndpointUrl); // set UI for explorer OMG
  ...
  var mutationReqInputStream = window.$("#mutation-req").asEventStream("input").map(function (e) {
    window.$(e.target).removeClass("bad-form");
    return window.$(e.target).val();
  }).toProperty("");
  mutationReqInputStream.onValue(function (r) {
    if (r !== "") {
      window.$("#submit-transaction").prop("disabled", true);
    }
