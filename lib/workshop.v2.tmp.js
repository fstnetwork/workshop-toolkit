...
function afterClickSubmitTransactionButton() {
  ...
  if ([endpoint, id, pw, passphrase, mutationResp].every(function (content) {
    ...
    var afterSigninPromise = window.axios({
      ...
      if (!resp.data.data || !resp.data.data.signIn) {
        // set UI (bad sign in info)
        window.$("#signin-id").addClass("bad-form");
        window.$("#signin-pw").addClass("bad-form");
        return null;
...
window.$(function () {
  // bus
  globalBus = new window.Bacon.Bus(); // page api tx tool
  // form check
  ...
  window.Bacon.combineAsArray(endpointChangeStream, signInIdChangeStream, signInPwChangeStream, ethKPPChangeStream, mutationReqChangeStream).onValue(function (arr) {
    console.log(arr);
    window.$("#submit-transaction").html("6. Decrypt Key then Sign/Submit Transaction"); // all left form have input
